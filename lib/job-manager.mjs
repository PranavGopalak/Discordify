...
function relaxDelay(currentMs, floorMs) {
  if (!currentMs || currentMs <= floorMs) {
    return floorMs;
  }
...
class DeletionJob {
  ...
  async tryUnarchiveThread(channelId) {
    if (this.blockedArchivedThreads.has(channelId)) {
      return false;
    }
    ...
    while (!this.stopRequested) {
      ...
      if (response.status === 429) {
        ...
        this.log('warn', `Thread unarchive was rate limited. Delete delay temporarily raised to ${waitMs}ms.`);
        await this.coolDown(waitMs);
        continue;
      }
  ...
  async runBulkTarget(target) {
    ...
    while (!this.stopRequested) {
      ...
      if (response.status === 429) {
        ...
        this.log('warn', `Search rate limited for ${target.label}. Search delay temporarily raised to ${waitMs}ms.`);
        await this.coolDown(waitMs);
        continue;
      }
      ...
      const searchSkippedCount = filtered.skippedMessages.length;
      this.progress.scanned += discoveredMessages.length;
      this.progress.matched += filtered.messagesToDelete.length;
      this.progress.skipped += searchSkippedCount;

      if (this.payload.previewOnly) {
        this.log(
          'debug',
          `Preview page in ${target.label}: ${filtered.messagesToDelete.length} matches, ${searchSkippedCount} skipped.`
        );
      ...
      let retainedOffset = searchSkippedCount;
      ...
      if (searchSkippedCount > 0) {
        this.log(
          'debug',
          `Skipped ${searchSkippedCount} non-target or non-deletable messages in ${target.label}.`
        );
      }
  ...
  async deleteWithRetry(message) {
    ...
    while (!this.stopRequested) {
      ...
      if (response.status === 429) {
        ...
        this.bumpThrottle(waitMs);
        this.deleteDelayMs = waitMs;
        this.log('warn', `Delete rate limited. Delete delay temporarily raised to ${waitMs}ms.`);
        await this.coolDown(waitMs);
        continue;
      }
      ...
      if (response.status === 400 && parsedBody?.code === 50083) {
        if (!attemptedUnarchive) {
          ...
          this.log('warn', `Message ${message.id} is inside an archived thread. Trying to unarchive ${message.channel_id}.`);
