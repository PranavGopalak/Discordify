...
class DeletionJob {
  ...
  start() {
    ...
    this.runPromise = this.run()
      ...
      .finally(() => this.releaseSecrets());
    return this;
  }

  releaseSecrets() {
    this.payload.token = '';
    if (this.client && typeof this.client.token === 'string') {
      this.client.token = '';
    }
  }
...
export class JobManager {
  ...
  prepareForJob() {
    this.trimJobs();
    if (Array.from(this.jobs.values()).some((job) => !TERMINAL_JOB_STATUSES.has(job.status))) {
      const error = new Error('Another preview or deletion job is already running. Stop it or wait for it to finish.');
      error.statusCode = 409;
      throw error;
    }
    if (this.jobs.size >= MAX_RETAINED_JOBS) {
      const error = new Error('The bounded job history is full.');
  ...
  trimJobs() {
    ...
    while (this.jobs.size >= MAX_RETAINED_JOBS && finished.length > 0) {
