...
export class DiscordClient {
  constructor(token, metricsSink, options = {}) {
    this.token = token;
    this.metricsSink = metricsSink;
    this.requestTimeoutMs = Math.min(
      REQUEST_TIMEOUT_MS,
      Math.max(1, Number(options.requestTimeoutMs) || REQUEST_TIMEOUT_MS)
    );
  }
  ...
  updateChannel(channelId, body, signal) {
    return this.raw(`/channels/${channelId}`, {
      method: 'PATCH',
      body,
      signal,
    });
  ...
  unarchiveThread(channelId, signal) {
    return this.updateChannel(channelId, { archived: false }, signal);
  }
}
